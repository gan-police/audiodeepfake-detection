...
from .learn_direct_train_classifier import get_model
from .models import save_model
from .optimizer import AdamCWT
from .prep_all import arange_datasets, get_all_labels, split_dataset_random
...
def _parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=8000,
        help="Sample rate of audio. (default: cmor4.6-0.87)",
    )
    parser.add_argument(
        "--f-min",
        type=float,
        default=2000,
        help="Minimum frequency to analyze. (default: 2 kHz)",
    )
    parser.add_argument(
        "--f-max",
        type=float,
        default=4000,
        help="Maximum frequency to analyze. (default: 4 kHz)",
    )
    parser.add_argument(
        "--data-prefix",
        type=str,
    ...
    parser.add_argument(
        "--direct-dl",
        action="store_true",
        help="If data is to be loaded directly from wavs.",
    )
    parser.add_argument(
        "--input-size",
        type=float,
        default=4400,
        help="Number of samples for each audio input.",
    )
    parser.add_argument(
        "--tensorboard",
        action="store_true",
...
def get_datasets(folder_list, window_size, train_size=0.7, val_size=0.1):
    """Get datasets from folder list with given size."""
    train_list, val_list, test_list = split_dataset_random(
        train_size, val_size, window_size, folder_list
    )
    train_labels = get_all_labels(train_list)
    val_labels = get_all_labels(val_list)
    test_labels = get_all_labels(test_list)

    train_list, val_list, test_list = arange_datasets(
        train_list, val_list, test_list, window_size
    )
    return train_list, val_list, test_list, [train_labels, val_labels, test_labels]
...
def create_data_loaders_learn(
    ...
    for data_prefix_el in data_prefix:
        ...
        if data_set_list is None:
            ...
            with torch.no_grad():
                train_data_set = LearnDirectDataset(  # type: ignore
                    data_set_list[0],
                    labels=labels[0],  # type: ignore
                    key=key,
            ...
            val_data_set = LearnDirectDataset(
                data_set_list[1],
            ...
            test_data_set = LearnDirectDataset(
                data_set_list[2],
                labels[2],  # type: ignore
                mean=mean,
                std=std,
                key=key,
                window_size=window_size,
    ...
    if len(data_set_list) == 1:  # type: ignore
        train_data_loader = DataLoader(
            train_data_set, batch_size=batch_size, shuffle=True, num_workers=num_workers
        )
        val_data_loader = DataLoader(
...
def main():
    ...
    if args.model == "learndeepnet":
        dataset_list = None
        labels = None
        folder_list = [
            Path("/home/s6kogase/data/fake/A_ljspeech/"),
            Path("/home/s6kogase/data/fake/B_melgan/"),
        ]
        if args.direct_dl:
            train_list, val_list, test_list, labels = get_datasets(
                folder_list, window_size=args.input_size
            )
            dataset_list = [train_list, val_list, test_list]
        train_data_loader, val_data_loader, test_data_set = create_data_loaders_learn(
            data_set_list=dataset_list,
            labels=labels,
            data_prefix=args.data_prefix,
            batch_size=args.batch_size,
            normal=args.calc_normalization,
        ...
        for it, batch in enumerate(
            ...
            if it % 10 == 0:
                print(
                    ...
                    acc.item(),
                    "bandwidth",
                    model.state_dict()["cwt.wavelet.bandwidth_par"].item() ** 2,
                    "center freq",
                    model.state_dict()["cwt.wavelet.center_par"].item() ** 2,
                    flush=True,
                )
